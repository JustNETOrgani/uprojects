...
contract AntiCounterfeit is AccessControl, ReentrancyGuard {
    ...
    mapping(uint256 => VerificationRecord[]) public productVerificationHistory;
    ...
    function verifyProduct(
        ...
        bool isAuthentic = performCounterfeitDetection(productId, qrCodeHash, location);
        ...
        productVerificationHistory[productId].push(newVerification);
     ...
     * @param qrCodeHash QR code hash for validation
     * @param location Verification location
    ...
    function performCounterfeitDetection(
        ...
        // 4. Check for suspicious verification patterns
        uint256 verificationCount = productVerificationHistory[productId].length;
        if (verificationCount > 100) {
            // Too many verifications might indicate a counterfeit being passed around
            return false;
        }
        
        // 5. Validate manufacturing date is reasonable
        if (product.manufacturingDate > block.timestamp) {
            return false; // Future manufacturing date is impossible
        }
        
        // 6. Check if product was registered too recently (potential fake)
    ...
    function updateProductLocation(
        ...
        uint256 locationUpdateId = _locationUpdateIds.current();
    ...
    /**
     * @dev Get product verification history
     * @param productId ID of the product
     * @return Array of verification records
     */
    function getProductVerificationHistory(uint256 productId) external view returns (VerificationRecord[] memory) {
        require(products[productId].isActive, "Product does not exist or is inactive");
